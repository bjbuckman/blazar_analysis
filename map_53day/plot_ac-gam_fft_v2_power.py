...
for ii in range(0,len(object_arr)):
	...
	try:
		...
		if obj_num >= 375 and obj_num < 800:
			ax0.scatter(l,b, alpha=excess, color=COLOR, marker=MARKER, zorder=ZORDER, s=SS)
		
		o_obj[counter] = obj_num
		counter+= 1
...
l_obj = l_obj[:counter]
b_obj = b_obj[:counter]
p_obj = np.log10(p_obj[:counter])

print('Most power in object '+str(o_obj[np.argmax(p_obj)]))
...
plot_name = './map_53day_gal_power_'
