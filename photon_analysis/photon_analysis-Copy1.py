...
for kk in range(0,len(object_arr)):  ## look at all objects 
    for mm in range(0,4): ## energy bins to look at
        ...
        d = {'time_mets':time_mets, 'time_days':time, 'dRA':dRA, 'dDE':dDE, 'theta1':theta_x, 'theta2':theta_y, 'energy':energy, 'angle_from_obj':angle_from_obj}
        ...
        #         blah = LombScargle(time_bin_samp_days[:num_bins_analyze], photon_grid[:,ii,jj]).power(freq)
                ...
                power_array_phase[ii,jj] = np.arctan2(model_params[2],model_params[1])
        ...
        plot_name = './instrumental/map_photon_phase_'+object+'_e'+str(ebin_number)+'_'
        for n in range(0,100):
        	if os.path.isfile(plot_name+str(n).zfill(2)+'.png'):
        		continue
        	else:
