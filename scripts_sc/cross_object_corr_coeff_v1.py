...
filecheck = SCRATCH_DIR+'/cross_object_gam+opt/cross_object_gam'+str(obj_gam).zfill(4)+'_opt'+str(obj_opt).zfill(4)+'_'+str(out_num).zfill(3)+'.dat' ## Filename to check
filename = './cross_object_gam+opt/cross_object_gam'+str(obj_gam).zfill(4)+'_opt'+str(obj_opt).zfill(4)+'_'+str(out_num).zfill(3)+'.dat' ## Filename to save to

if (not os.path.isfile(filename)) or (RERUN == 1):
	...
	for ip in range(0,num_iterations):
		...
		correlation = np.concatenate((correlation_b[:-1], correlation_f), axis=1)
		
		CORRELATION[ip*num_redux+1:ip*num_redux+num_redux+1] = correlation
