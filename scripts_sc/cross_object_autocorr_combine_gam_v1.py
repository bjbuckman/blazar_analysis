...
gam_num = int(sys.argv[3]) ## gamma object number
...
for ii in range(0,len(opt_num)):
	if not (opt_num[ii] == gam_num):
		...
		if opt_num[ii] < gam_num:
			filename = './cross_object_gam+gam/cross_object_gam'+str(opt_num[ii]).zfill(4)+'_gam'+str(gam_num).zfill(4)+'_'+str(out_num).zfill(3)+'.dat'
		else:
			filename = './cross_object_gam+gam/cross_object_gam'+str(gam_num).zfill(4)+'_gam'+str(opt_num[ii]).zfill(4)+'_'+str(out_num).zfill(3)+'.dat'
...
output_filename = 'cross_stats_gam+gam_object'+str(gam_num).zfill(4)+'_'+str(out_num).zfill(3)+'.dat'
