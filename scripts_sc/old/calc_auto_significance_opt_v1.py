...
# flux_err_2d = 1./flux_err_d**2+1.e-50
# flux_err_2u = 1./flux_err_u**2+1.e-50
...
# for ii in range(0,tnum):
	...
	# if np.any(t_arr):
		# f[ii] = np.mean(flux[t_arr])
		# fe_d[ii] = np.sum(flux_err_2d[t_arr])**-0.5
		# fe_u[ii] = np.sum(flux_err_2u[t_arr])**-0.5
...
# for ii in range(0,tnum):
	...
	# if np.any(t_arr):
		...
		# if f[ii] != -100:
			# if f[ii]-f_mean > 0.:
				# F[ii] = (f[ii]-f_mean)/fe_u[ii]
			# else:
				# F[ii] = (f[ii]-f_mean)/fe_d[ii]
			# Fe[ii] = 1.
# t_gam = t
# F_gam1 = F
# fe_gam1 = Fe
...
for ip in range(0,num_iterations):
	...
	fe_gam[t_gam_arr_use] = fe_gam1[t_gam_arr]
	...
	for ii in range(0,Tnum):
		...
		if int(num_points_f[ii]) > 3:
			...
			arr_gam_err = arr_gam_err[bool_arr]
...
filename = './autocor_sig/autocor_sig_opt_'
