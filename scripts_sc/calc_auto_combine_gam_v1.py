...
directory_cor = './autocor_err/autocor_err_gam_'
# directory_sig = './autocor_sig/autocor_sig_gam_'

## Combine correlation coefficient into single list
for ii in range(0,len(corr_coeff_file)):
	filename =  directory_cor+str(corr_coeff_file[ii]).zfill(3)+'.dat'
...
# for ii in range(0,len(corr_signi_file)):
	# filename = directory_sig+str(corr_signi_file[ii]).zfill(3)+'.dat'
	# read_in = np.loadtxt(filename)
...
## Output
output_filename = 'object'+str(obj_num).zfill(4)+'_auto_gam_stats'+str(iout).zfill(3)+'.dat'
