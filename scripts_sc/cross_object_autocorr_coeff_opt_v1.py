...
if obj_gam < obj_opt:
	filecheck = SCRATCH_DIR+'/cross_object_opt+opt/cross_object_opt'+str(obj_gam).zfill(4)+'_opt'+str(obj_opt).zfill(4)+'_'+str(out_num).zfill(3)+'.dat' ## Filename to save to
	filename = './cross_object_opt+opt/cross_object_opt'+str(obj_gam).zfill(4)+'_opt'+str(obj_opt).zfill(4)+'_'+str(out_num).zfill(3)+'.dat' ## Filename to save to
else:
	filecheck = SCRATCH_DIR+'/cross_object_opt+opt/cross_object_opt'+str(obj_opt).zfill(4)+'_opt'+str(obj_gam).zfill(4)+'_'+str(out_num).zfill(3)+'.dat' ## Filename to save to
	filename = './cross_object_opt+opt/cross_object_opt'+str(obj_opt).zfill(4)+'_opt'+str(obj_gam).zfill(4)+'_'+str(out_num).zfill(3)+'.dat' ## Filename to save to
...
if (not os.path.isfile(filecheck)) or (RERUN == 1):
	...
	read_in = pd.read_csv(gamma_file)
	...
	for ip in range(0,num_iterations):
		...
		for ii in range(0,tnum):
			...
			## Gamma-ray
			if F_gam[ii] != -100:
				F1_gam[ii] = np.random.normal(loc=F_gam[ii], scale=fe_gam[ii])
			
			## Optical
			if F_opt[ii] != -100:
				F1_opt[ii] = np.random.normal(loc=F_opt[ii], scale=fe_opt[ii])
