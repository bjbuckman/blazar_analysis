...
for ii in range(0,len(object_arr)):
	...
	try:
		...
		expo = np.mean(expo, axis=1)
		num_e = counts.shape[1]
	...
	if gamma_tag == 1 and gamma_flux_tag == 1:
		ax00.errorbar(expo, F_gam_d, yerr=[fe_gam_dd,fe_gam_ud], fmt='o', markersize=MSIZE, alpha=ALPHA, elinewidth=ELINE, capsize=0.5, capthick=0.5, color=cmap2(0.1), zorder=21)
		
		gam_min = np.percentile(F_gam_d[F_gam_d>0], 10)
		gam_max = np.amax(F_gam_d + fe_gam_ud)

		ax00.set_ylim([gam_min/2.,gam_max*1.5])
		ax00.set_xlim([1.e7, 3.e8])
		...
		dexpo = expo[26:]-expo[:-26]
		dF_gam_d = F_gam_d[26:]-F_gam_d[:-26]
		dfe_gam_dd = np.sqrt(fe_gam_dd[26:]**2 + fe_gam_dd[:-26]**2)
		dfe_gam_ud = np.sqrt(fe_gam_ud[26:]**2 + fe_gam_ud[:-26]**2)
		ax01.errorbar((dexpo), (dF_gam_d/(F_gam_d[26:]+F_gam_d[:-26])*2), yerr=[dfe_gam_dd,dfe_gam_ud], fmt='o', markersize=MSIZE, alpha=ALPHA, elinewidth=ELINE, capsize=0.5, capthick=0.5, color=cmap2(0.1), zorder=21)
		# ax01.set_xscale('log')
		# ax01.set_yscale('log')
		gam_min = np.percentile(F_gam_d[F_gam_d>0], 10)
		gam_max = np.amax(F_gam_d + fe_gam_ud)
	...
	if gamma_flux_tag == 1:
		for jj in range(0,6):#num_e):
			factor = 1.#10**(2*jj)
			ax10.scatter(expo, counts[:,jj]*factor, marker='o', s=MSIZE, alpha=ALPHA, color=cmap2(jj/7), zorder=21)
			
		ax10.set_xscale('log')
		ax10.set_yscale('log')
		ax10.set_xlim([1.e7, 3.e8])
		# ax10.set_xlim(axN0_limits)
		
		
		cohere = coherence(F_gam_d, expo)
		
		ax11.plot(cohere[0], cohere[1])
		ax11.set_yscale('log')
		
		
		# dexpo = expo[26:]-expo[:-26]
		# dflux = flux[26:]-flux[:-26]
		# dfe_gam_dd = np.sqrt(flux_err_d[26:]**2 + flux_err_d[:-26]**2)
		# dfe_gam_ud = np.sqrt(flux_err_u[26:]**2 + flux_err_u[:-26]**2)
		# for jj in range(0,6):#num_e):
			# factor = 1.#10**(2*jj)
			# ax11.errorbar(dexpo, dflux[:,jj], yerr=[dfe_gam_dd[:,jj],dfe_gam_ud[:,jj]], fmt='o', markersize=MSIZE, alpha=ALPHA, elinewidth=ELINE, capsize=0.5, capthick=0.5, color=cmap2(jj/7), zorder=21)
	...
	plt.tight_layout()
	plot_name = '../analysis/full_gamma+expo_correlation/plot_cor_object'+str(obj_num).zfill(4)+'_'
