...
for ii in range(0,len(object_arr)):
	...
	gamma_tag = 1
	try:
		gamma_file = '../data/gamma-ray/object'+str(obj_num).zfill(4)+'_gam.npy'
	...
	if gamma_tag == 1:
		for jj in range(0,min(enum,4)):
			factor = 10**(2*jj)
			if MODEL == 0:
				day_color = time/np.amax(time)
				ax0.scatter(expo[:,jj], counts[:,jj]+0.2*jj, color=cmap2(jj/min(enum,4)), alpha=ALPHA, s=0.2)
			elif MODEL == 1:
				ax0.errorbar(expo[:,jj], flux[:,jj], yerr=[flux_ll[:,jj], flux_ul[:,jj]], alpha=ALPHA, color=cmap2(jj/min(enum,4)), markersize=0.2, fmt='o', elinewidth=0.2)
			# elif MODEL == 2:
				# ax0.scatter(time[:], counts_expo[:,jj]*factor, alpha=ALPHA, color=cmap2(jj/enum), s=0.2)
			# elif MODEL == 3:
				# ax0.scatter(time[:], counts[:,jj]*factor/np.average(counts[:,jj]), alpha=ALPHA, color=cmap2(jj/4.), s=0.2, marker='o')
				# ax0.scatter(time[:], expo[:,jj]*factor/np.average(expo[:,jj]), alpha=ALPHA, color=cmap2((jj+0.5)/4.), s=0.2, marker='x')
	...
	# ax0.set_ylim([opt_min*0.66,opt_max*1.5])
	# ax1.set_ylim([gam_min*0.66,gam_max*1.5])

	# ax0.set_ylim([-1,20])
	# ax0.set_xlim([0,0.75e8])
	...
	if MODEL == 0:
		ax0.set_ylim([-1,20])
		ax0.set_xlim([0,0.75e8])
		ax0.set_xlabel(r'Exposure [cm$^{-2}$ s$^{-1}$]')
		ax0.set_ylabel(r'Counts []')
	elif MODEL == 1:
		ax0.set_xlabel(r'Exposure [cm$^{-2}$ s$^{-1}$]')
	...
	if ZOOM == 0:
		...
		if MODEL == 0:
			plot_name = '../analysis/full_expo-counts_scatter/plot_object'+str(obj_num).zfill(4)+'_'
		elif MODEL == 1:
			plot_name = '../analysis/full_expo-flux_scatter/plot_object'+str(obj_num).zfill(4)+'_'
		# elif MODEL == 2:
		...
		ii_max = int(4000/150+1)
		for ii in range(0,ii_max):
			ax0.set_xlim([150*ii,150*ii+300])
			ax1.set_xlim([150*ii,150*ii+300])
