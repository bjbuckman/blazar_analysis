...
for ii in range(0,len(object_arr)):
	...
	if gamma_tag == 1:
		for jj in range(0,8):#,min(enum,1)):
			factor = 10**(3*jj)
			ax0.errorbar(time[:], flux[:,jj]*factor, yerr=[flux_err_d[:,jj]*factor,flux_err_u[:,jj]*factor], fmt='o', markersize=MSIZE, alpha=ALPHA, elinewidth=ELINE, capsize=0.5, capthick=0.5, color=cmap2(jj/enum), zorder=21)
	...
	ax0.set_ylabel(r'$E\Phi$ [MeV cm$^{-2}$ s$^{-1}$]')
	...
	if ZOOM == 0:
		...
		plot_name = '../analysis/full_gamma_curves/plot_lc_object'+str(obj_num).zfill(4)+'_'
		...
		for ii in range(0,ii_max):
			...
			plot_name = '../analysis/full_gamma_curves/plot_lc_object'+str(obj_num).zfill(4)+'_zoom'+str(ii).zfill(2)+'_'
			for n in range(0,100):
				if os.path.isfile(plot_name+str(n).zfill(2)+'.png'):
					continue
				else:
