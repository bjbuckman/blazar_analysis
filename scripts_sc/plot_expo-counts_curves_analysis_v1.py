...
for ii in range(0,len(object_arr)):
	...
	arr_keep = np.logical_and( expo[:,0] > np.percentile(expo[:,0], 5.), expo[:,0] < np.percentile(expo[:,0], 95.))
	
	time = time[arr_keep]
	flux = flux[arr_keep]
	flux_ll = flux_ll[arr_keep]
	flux_ul = flux_ul[arr_keep]
	expo = expo[arr_keep]
	counts = counts[arr_keep]
	...
	if gamma_tag == 1:
		for jj in PLOT_RANGE: #min(enum,3)):
			enum=3+jj
			factor = 1.# 10**(2*jj)
			if MODEL == 0:
				...
				ax0.plot(time[:], expo_normalized/expo_normalized, alpha=ALPHA, color=cmap2((jj+0.5)/enum))#, s=0.2, marker='x')
				ax0.plot(time[:], expo_normalized/np.average(counts[:,jj]), alpha=ALPHA, color=cmap2((jj+0.5)/enum), linestyle='--')#, s=0.2, marker='x')
				
				ax0.fill_between(time[:], (expo_normalized-np.sqrt(expo_normalized))/expo_normalized, (expo_normalized+np.sqrt(expo_normalized))/expo_normalized, alpha=0.2, edgecolor=cmap2((jj+0.5)/enum), color=cmap2((jj+0.5)/enum))
				ax0.fill_between(time[:], (expo_normalized-2.*np.sqrt(expo_normalized))/expo_normalized, (expo_normalized+2.*np.sqrt(expo_normalized))/expo_normalized, alpha=0.05, edgecolor=cmap2((jj+0.5)/enum), color=cmap2((jj+0.5)/enum))
				
				ax0.scatter(time[:], counts[:,jj]*factor/expo_normalized, alpha=ALPHA, color=cmap2(jj/enum), s=0.2, marker='o')
				
				rho, alpha = pearsonr(expo_normalized/np.average(counts[:,jj]), counts[:,jj]*factor/expo_normalized)
				rhos, alphas = spearmanr(expo_normalized/np.average(counts[:,jj]), counts[:,jj]*factor/expo_normalized)
				print(obj_num, rho, alpha, rhos, alphas)
	...
	plot_min = 0. #np.percentile(counts[:,3]/expo_normalized, 5)/2.
	plot_max = 4. #np.percentile(counts[:,0]/expo_normalized, 95)*2.
	...
	if ZOOM == 0:
		...
		for ii in range(0,ii_max):
			...
			for jj in PLOT_RANGE:
				arr = np.logical_and(time > 300*ii, time < 300*ii+400)
				expo_normalized = expo[arr,jj]*factor/np.average(expo[:,jj])*np.average(counts[:,jj])
				rho, alpha = pearsonr(expo_normalized/np.average(counts[:,jj]), counts[arr,jj]*factor/expo_normalized)
				rhos, alphas = spearmanr(expo_normalized/np.average(counts[:,jj]), counts[arr,jj]*factor/expo_normalized)
				print(obj_num, jj, rho, alpha, rhos, alphas)
