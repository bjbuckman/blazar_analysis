...
calc_flux() {
	...
	cp $fermi_dir/step11_calc_total_flux_vT.py $TMPDIR/work_dir${NAME}
	cp ${NAME}.tar.gz ${NAME}_out7_rois.txt $TMPDIR/work_dir${NAME}
	cd $TMPDIR/work_dir${NAME}
	
	tar -xf ${NAME}.tar.gz
	
	python step11_calc_total_flux_vT.py ${NAME}_out7_rois.txt >> ${NAME}_fermi_tot.dat
	cp *.dat *.npy $fermi_dir/fluxes/vTT
	echo $NAME
}
